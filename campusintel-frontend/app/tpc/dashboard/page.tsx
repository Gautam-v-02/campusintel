    ...
    <div className="flex min-h-screen bg-[#07070f]">
      ...
      <aside className="w-[220px] min-h-screen bg-[#0a0a14] border-r border-[#1e1e30] flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {['📊 Overview', '👥 Students', '🏢 Drives', '🔔 Alerts', '📋 Reports'].map((item, i) => (
            <div key={i} className={`flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm cursor-pointer transition ${
              i === 0 ? 'bg-indigo-500/10 text-indigo-300 border-l-2 border-indigo-500' : 'text-[#6b7280] hover:text-[#c4c4d8] hover:bg-white/5'
            }`}>
              {item}
            </div>
          ))}
