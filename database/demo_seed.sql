...
-- Register all 3 for the Google demo drive (use subqueries to get real IDs by email)
INSERT INTO student_registrations (student_id, drive_id, college_id, status)
SELECT id, 'demo-drive-google', 'college-lpu-001', 'registered'
FROM users WHERE email = 'priya@lpu.in'
ON CONFLICT DO NOTHING;

INSERT INTO student_registrations (student_id, drive_id, college_id, status)
SELECT id, 'demo-drive-google', 'college-lpu-001', 'registered'
FROM users WHERE email = 'arjun@lpu.in'
ON CONFLICT DO NOTHING;

INSERT INTO student_registrations (student_id, drive_id, college_id, status)
SELECT id, 'demo-drive-google', 'college-lpu-001', 'registered'
FROM users WHERE email = 'meera@lpu.in'
ON CONFLICT DO NOTHING;
...
SELECT
  'college-lpu-001', 'company-google-001', u.id,
  'technical_1',
  ...
  true
FROM users u WHERE u.email = 'priya@lpu.in';

-- Extra Google debrief from Meera
INSERT INTO interview_debriefs 
(college_id, company_id, student_id, round_type, questions_asked, outcome,
 difficulty_rating, extracted_topics, success_factors, is_verified)
SELECT
  ...
  NULL, true
FROM users u WHERE u.email = 'meera@lpu.in';

-- Seed strategy weights so SELECT_STRATEGY step shows real data
INSERT INTO strategy_weights (college_id, company_id, student_profile_type, strategy, weight, win_rate, times_used)
VALUES
...
DO UPDATE SET weight = EXCLUDED.weight, win_rate = EXCLUDED.win_rate, times_used = EXCLUDED.times_used;
